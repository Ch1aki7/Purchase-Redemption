...
- 多月份滚动回测与日历趋势建模
- 月度总量/日内形状申购模型 + 节假日距离赎回模型（当前唯一正式组合）
- 2014 年 8 月验证集评估
- 2014 年 9 月滚动预测
- Streamlit 可视化展示
- 课程需求分析文档
...
│   ├── train.py / predict.py                 # 生成组合模型所需的内部滚动基线
│   ├── calendar_forecast.py                  # 日历与趋势基线
│   ├── monthly_shape_forecast.py             # 月度总量/日内形状模型
│   ├── event_distance_forecast.py            # 节假日距离与周周期模型
│   ├── online_ablation.py                    # 组合申购与赎回候选
│   ├── production.py                         # 发布唯一正式结果及匹配验证集
...
1. 经营总览：展示未来申购、赎回、净流量、赎回峰值以及基于近期历史分位数生成的流动性关注清单
2. 数据洞察：支持日期区间筛选，展示历史资金趋势、星期周期、用户分布、收益率、Shibor 和数据质量，并可下载筛选结果
3. 模型与验证：说明当前正式组合结构，展示与该结构一致的独立验证指标和真实值对比
4. 预测与计划：提供赎回压力增幅、准备金安全垫两项情景参数，生成每日风险等级和建议准备金，支持下载提交文件与资金计划
5. 误差诊断：展示验证集绝对误差、相对误差、每日得分、零分日、高估/低估方向和改进提示

侧边栏提供“一键运行完整流程”按钮，会执行 `run_all.py`，依次完成内部基线、滚动回测、日历趋势、月度形状、事件距离和正式发布。前端只展示正式组合模型，不再提供旧模型切换入口。
...
项目已集成端到端流程：数据预处理 → 内部滚动基线 → 多月份回测选参 → 日历趋势 → 月度总量/日内形状 → 节假日距离与周周期 → 线上验证有效的目标级组合 → 可视化展示。正式提交采用“月度形状申购 + 事件距离赎回”，前端验证文件由同一预测结构生成。
